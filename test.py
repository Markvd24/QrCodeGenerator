...
exp_add = lambda a, b: integer_to_exponent[exponent_to_integer[a] ^ exponent_to_integer[b]]
def generate_generator_polynomial(type):
    ...
    for i in reversed(range(1, type)):
        factor1 = previous_polynomial[i] + (type - 1) # x(i)a(old) * x(i)(new)
        factor2 = previous_polynomial[i - 1] # x(i-1)a(n) * x(1)a(0)

        new_polynomial.append(exp_add(factor1, factor2))

    # add x(0)a(old) * x(0)a(new)
    new_polynomial.append(previous_polynomial[type-1] + (type - 1))

    return new_polynomial


print(generate_generator_polynomial(10))
generator_polynomial = generate_generator_polynomial(10)
